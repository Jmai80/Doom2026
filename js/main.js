...
import { keys, cursors, initInput, touchState,
         consumeFireTap, isTouchDevice }     from './input.js';
import { handleInput }                       from './player.js';
import { updatePeekers, peekers,
         nearAimTarget }                     from './peekers.js';
...
function update(_time, deltaMs) {
  ...
  if (state.phase === 'playing') {
    // Sticky aim (endast touch): halverad vridkänslighet när siktet är
    // nära en fiende — tummen sveper inte förbi målet lika lätt.
    const aimSlow = (isTouchDevice && nearAimTarget()) ? 0.5 : 1;
    handleInput(dt, aimSlow);
